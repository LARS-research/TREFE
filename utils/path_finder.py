...
def str_to_path(path_str):
    ...
    for c in path_str:
        if c == 'A':
            path.append(path[-1] + np.array([1, 1]))
        elif c == 'B':
...
class PathFinder:
            ...
            for path in self.all_paths:
                for i in range(1, len(path)):
                    ...
                    if cur_node_str not in self._nodes_indegree_map:
                        self._nodes_indegree_map[cur_node_str] = set()
                        self._nodes_indegree_map[cur_node_str].add(pre_node_str)
                    else:
                        self._nodes_indegree_map[cur_node_str].add(pre_node_str)
    ...
    def choice_random_path(self):
        path_id = np.random.randint(0, self.n_paths)
        return self.all_paths[path_id], path_id 
...
if __name__ == "__main__":
    ea_pathfinder = PathFinder()
    print(ea_pathfinder.nodes_indegree_map)
    print(ea_pathfinder.topological_sorted_nodes)
