...
from datasets import Collate, LabelMap
from datasets.general_hw import apply_data_augmentation
from utils import (AverageMeter, generate_distort, generate_perspective,
                   generate_stretch, get_logger)
...
class RIMESDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, label_map, root_dir, mode='train', input_shape=[1, 64, 1200], p_aug=-1):
        ...
        self.tag_file = os.path.join(root_dir, 'all_tags.txt')
        self.img_paths = []
        self.labels    = []
        with open(self.tag_file, 'r') as f:
            lines = f.read().strip().split('\n')
            for line in lines:
                set_name, save_path, label = line.split(' ', 2)
                if set_name == mode:
                    assert os.path.exists(save_path), '%s not exists' % save_path
    ...
    def aug_norm_img(self, img):
        ...
        if self.p_aug > 0:
            ...
            if np.random.randn() < self.p_aug:
                imgN, _ = apply_data_augmentation(imgN, rimes_augmentation)
                imgN = imgN[:, :, 0]
...
def build_data(cfg, logger=None):
    ...
    label_map = LabelMap(character_set_file=os.path.join(root_dir, 'charset.txt'))
    train_dataset = RIMESDataset(label_map,
        root_dir=root_dir,
        mode='train',
    ...
    val_dataset = RIMESDataset(label_map,
        root_dir=root_dir,
        mode='valid',
        input_shape=cfg.DATASETS.INPUT_SHAPE)
    ...
    test_dataset = RIMESDataset(label_map,
        root_dir=root_dir,
        mode='test',
        input_shape=cfg.DATASETS.INPUT_SHAPE)
...
def parser_args():
    ...
    cfg.output_dir = '../work_dirs/RIMES/optimal_compact'
    cfg.init_lr = 0.8
    cfg.weight_decay = 1e-5
    cfg.n_warmup_epochs = 10
    cfg.n_classes = 101    # 0 = blank 1:101 = class 
    cfg.print_every = 100
    # transformer
    cfg.transformer = CN()
...
if __name__ == '__main__':
    ...
    latency = compute_latency_ms_tensorrt(onnx_file, input_size=input_size, num_iters=1000, verbose=False, mode='FP16')
    print(latency)
