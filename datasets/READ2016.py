...
class READ2016Dataset(torch.utils.data.Dataset):

    def __init__(self, label_map, root_dir, mode='train', input_shape=[1, 64, 1200], p_aug=-1):
        super(READ2016Dataset, self).__init__()
        ...
        for image_path, label in samples.items():
            image_path = os.path.join(root_dir, mode, image_path)
    ...
    def aug_norm_img(self, img):
        ...
        def add_distort(img):
            if np.random.rand() < self.p_aug:
                img = np.pad(img, ((5, 5), (5, 5), (0, 0)), mode='constant', constant_values=0)
        ...
        if self.p_aug > 0:
            imgN = np.pad(imgN, ((2, 2), (2, 2), (0, 0)), mode='constant', constant_values=0).astype('uint8')
